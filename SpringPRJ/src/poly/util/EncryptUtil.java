...
public class EncryptUtil {
	...
	/*
	 * 암호화 알고리즘에 추가시킬 암호화문구
	 * 일반적인 암호화 알고리즘 sha-256을 통해서만 암호화 시킬 경우, 암호화 된 값만 보고 일반적인 비밀번호에 대한 값을 쉽게
	 * 예측 가능함에 따라서 암호화할 때 암호화되는 값에 추가적인 문자열을 붙혀서 함께 함호화를 진행함.
	 */
	
	final static String addMessage = "PolyDataAnalysis";
	
	/*
	 * AES-128-CBC 암호화 알고리즘에 사용되는 초기 백터와 암호화 키
	 * 
	 */
	
	//초기 백터(16바이트 크기를 가지며, 16바이트 단위로 암호화 시, 암호화할 총 길이가 16바이트가 되지 못하면 뒤에 추가하는 바이트)
	final static byte[] ivBytes = { 0x00, 0x00 ,0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
			0x00, 0x00, 0x00, 0x00};
	...
	public static String enHashSHA256(String str) throws Exception{
		...
		try {
			...
			//여러 해시 알고리즘중 가장 많이 사용되는 SHA-256을 지원하고있음
